...
async function recordOctokit(name: string, token: string): Promise<InstanceType<typeof GitHub>> {
  ...
  const folder = join(dirname(callerFile), "__fixtures__");
...
async function writeReplay(path: FileHandle, replay: Replay): Promise<void> {
...
async function replayOctokit(name: string, token: string): Promise<InstanceType<typeof GitHub>> {
...
async function readReplay(rl: readline.Interface): Promise<Replay> {
  ...
  const method = lines[0] as RequestMethod | undefined;
  const path = lines[1];
  const url = lines[2];
  const statusStr = lines[3];
  const dataStr = lines[4];

  if (!method || !path || !url || !statusStr || !dataStr) {
    throw new Error("replay: incomplete replay record");
  }
  ...
  return {
    ...
    data: JSON.parse(Buffer.from(dataStr, "base64").toString()) as unknown,
  };
}

async function oneLine(rl: readline.Interface): Promise<string> {
